...
무향그래프에서 
특정 정점 u가 단절점이 되려면 u의 자손들과 u의 조상들이 서로 연결되어있어야 한다.
...
즉 루트인 정점은 단절점의 여부가 자손의 수로 결정해야 한다. (최소 높이로 판단하게 되면 u가 스패닝 트리의 루트라면 아무리 높이 올라가도 u까지밖에 못온다. 그러면 위의 기준대로
하게 되면 u의 위로는 못올라가니깐 절단점이 무조건 아니라고 생각하게 된다.)
...
int findCutVertex(int here, bool isRoot) {
	...
	//우선은 자기자신이 아직까지는 갈 수있는 가장빠른 발견순서
	...
	for (int i = 0; i < adj[here].size(); ++i) {
		...
		if (discovered[there] == -1) {
			...
			//만약 위에서 얻은 번호가 here보다 더 낮은 높이에 있는 친구라면  here은 절단점이야 (루트가 아니고)
			if (!isRoot && subtree >= discovered[here])
				isCutVertex[here] = true;

			ret = min(ret, subtree);		//여기 친구도 반환값을 찾는 중이야 (here를 루트로 한 서브트리에서 올라갈 수 있는 제일 높은 친구를 찾는다.)
		}
		else {
...
만약에 forward이거나 back edge라면 u와 v를 잇는 또다른 경로가 존재한다는 의미가 되는데 그렇게 되면 이 간선은 절대로 bridge가 될 수 없지
...
따라서 tree edge만을 골라주면 된다. (무방향 그래프일때는 cross edge는 무시하면 되니깐)
...
int dfs(int A, int parent) {
	...
	for (int i = 0; i < adj[A].size(); ++i) {
		...
		if (!discovered[next]) {
			...
			//이미 방문한 정점이다.... ->A의 forward/back edge중에 하나겠지? discovered[next] 가 더 작다면 조상이니깐 backedge이고 크다면 forward edge일 꺼야
