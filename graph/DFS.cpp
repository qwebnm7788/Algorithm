...
--> 즉 next에서 들어오던가 here로 들어오던가 서로 도달할 수 있다. (사이클은 하나를 타고 쭉 돌면 시작점으로 돌아올수 있는지의 여부이므로 사이클에 속함을 알 수 있다.)
3. visited[next] = false, finished[next] =true
--> 이런 경우는 없어 왜냐면 시작을 안했는데 끝이 났다는 거니깐  (코드 자체로 불가능하다)
4. visited[next] = true, finished[next] = true
--> next는 이미 모든 탐색이 끝난것이므로 (DFS로 다 뒤진거) here이랑 next는 같은 싸이클에 있을 수 없다. (즉 다시보면 here에서는 next에 도달 가능한데, next에서는 here로 올 수 없었다는 것이 된다.)
*/
...
void dfs2(int here) {
	...
	for (int i = 0; i < adj[here].size(); ++i) {
		...
		//there은 이미 방문한 정점인데 + here보다는 늦게 방문된 정점이라면 (순방향 간선)
		...
		//there은 이미 방문한 정점이고 here보다 먼저 방문된 정점일때
			...
			//재귀적으로 타게 되면 결국에는 u를 만나게 될꺼고(사이클이라면) 그때 만난 u는 맨처음 시작에서 이미 열렸으므로
			//discovered =true 이고 finished = false인 역방향 간선으로 분류가 될꺼야
			//그러니깐 이렇게 재귀적으로 돌다가 역방향 간선들은 모두 한 사이클에 들어가는 친구야
			//왜냐면 맨 처음으로 여기에 도달한 친구를 끝으로하는 사이클이 생기기 때문 (u -> ... -> here -> u 이므로 여기 까지가 딱 사이클이다)
			cout << "back edge" << endl;
		}
		else {
...
//무방향 그래프에서 사이클 찾기
//dfs에서 자신의 바로 위 부모를 제외하고 이미 방문한 정점에 도달한다면 사이클을 갖는다. ,즉 역방향 간선을 가지면
...
boolean DFS(v){
	...
	for( each vertex w adjacent to v){
		...
		if(color[w] == 'white'){
			...
			//이미 방문한 점이고 그 점이 나의 부모가 아니라면(부모라면 그냥 이어진 친구라고 볼수도 있으므로 사이클은 아니야 단순 u->v 간선)
			return true;	//참
		}
	}
